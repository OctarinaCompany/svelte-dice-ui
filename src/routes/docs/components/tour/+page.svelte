<script lang="ts">
	...
	const rootProps = [
		...
		},
		{
			...
			type: '() => void',
			default: '—',
			description: 'Fires once when the visitor finishes the last step. Never paired with `onSkip`.'
		},
		{
			prop: 'onSkip',
			type: '() => void',
	...
	const controlProps = [
		...
		},
		{
			...
			description: '`Tour.Prev` and `Tour.Skip` default to `"outline"`, `Tour.Next` to `"default"`.'
		},
		{
			prop: 'size',
			type: 'ButtonSize',
...
<svelte:head>
	<title>Tour — svelte-dice-ui</title>
</svelte:head>

<article class="flex flex-col gap-10">
	<div class="flex flex-col gap-2">
		<h1 class="text-3xl font-semibold tracking-tight">Tour</h1>
		<p class="text-muted-foreground">
			A guided tour component that highlights elements and provides step-by-step instructions to
			help users learn about your application.
		</p>
	</div>

	<ComponentPreview
		title="Default"
		description="Mirrors tour-demo.tsx — a four-step dashboard walkthrough with one shared footer, a spotlight backdrop, a ring and an arrow from the second step on."
	>
		<div class="flex flex-col items-center gap-8 p-4">
			<div class="flex flex-col items-center gap-4">
				<div class="flex flex-col items-center gap-1">
					...
					<p class="text-center text-muted-foreground">Take a quick tour to explore key features</p>
				</div>
	...
	<ComponentPreview
		title="Controlled"
		description="Mirrors tour-controlled-demo.tsx — external Start / Prev / Next buttons own the state, a fourth target only appears once the tour reaches step 3, and the parts render inline without a portal."
	>
		<div class="flex flex-col items-center gap-8 p-4">
			<div class="flex flex-col items-center gap-4">
