<script lang="ts">
	...
	/** A license plate: three letters then four digits (mask-input-custom-pattern-demo.tsx). */
...
<svelte:head>
	<title>Mask Input — svelte-dice-ui</title>
</svelte:head>

<article class="flex flex-col gap-10">
	<div class="flex flex-col gap-2">
		<h1 class="text-3xl font-semibold tracking-tight">Mask Input</h1>
		<p class="text-muted-foreground">
			An input that formats what you type against a pattern like a phone number, date, credit card
			or currency amount — and keeps the caret where you left it.
		</p>
	</div>

	<ComponentPreview
		title="Default"
		description="Mirrors mask-input-demo.tsx. Six built-in patterns, each reporting both the masked and the unmasked value."
	>
	...
	<ComponentPreview
		...
		description="Mirrors mask-input-custom-pattern-demo.tsx. A mask object brings its own pattern, transform and validate — the product code even prepends its PRD prefix."
	>
	...
	<ComponentPreview
		...
		description="Mirrors mask-input-validation-modes-demo.tsx. The same phone mask under four modes — onSubmit never calls onValidate, so its card validates in the submit handler instead."
	>
				...
				>
					...
					{#if mode.value === 'onSubmit'}
						<form onsubmit={onModeSubmit} class="flex flex-col gap-2">
							{@render inputContent()}
							<Button type="submit" size="sm" class="h-7 text-xs">Submit</Button>
						</form>
					{:else}
						{@render inputContent()}
					{/if}
					<div class="flex items-center gap-1">
						<Badge variant={state.isValid ? 'default' : 'destructive'} class="h-5 px-1.5 text-xs">
	...
	<ComponentPreview
		...
		description="Mirrors mask-input-card-information-demo.tsx. Three masks — creditCard with its Luhn check, creditCardExpiry, and a custom three-digit CVC — all validating on blur."
	>
	...
	<ComponentPreview
		title="With form"
		description="Mirrors mask-input-form-demo.tsx. A plain <form> with Field.FieldGroup and rune state stands in for react-hook-form and zod, which have no Svelte analogue — each field stores its unmasked value and is checked on submit."
	>
	...
	<section class="flex flex-col gap-6">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">MaskInput</h3>
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">Built-in patterns</h3>
			<p class="text-sm text-muted-foreground">
