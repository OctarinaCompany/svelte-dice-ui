<script lang="ts">
	...
	const level = $derived(
		QRCode.QR_CODE_LEVELS.find((candidate) => candidate === playgroundLevel) ?? QRCode.DEFAULT_LEVEL
	);
	...
	const levels = [
		{ level: 'L', recovery: '~7%', description: 'Low — the densest, least resilient code.' },
...
<svelte:head>
	<title>QR Code — svelte-dice-ui</title>
</svelte:head>
...
<article class="flex flex-col gap-10">
	<div class="flex flex-col gap-2">
		<h1 class="text-3xl font-semibold tracking-tight">QR Code</h1>
		<p class="text-muted-foreground">
			A flexible QR code component for generating and displaying QR codes with customization
			options.
		</p>
	</div>
	...
	<ComponentPreview title="Default" description="Mirrors qr-code-demo.tsx.">
	...
	<ComponentPreview
		...
		description="Mirrors qr-code-formats-demo.tsx. Each code renders through a different part, and each download button is composed onto a Button through the child snippet — the port of upstream's asChild."
	>
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
			...
			<div class="flex flex-col items-center gap-2">
				<QRCode.Root value={VALUE} size={120}>
					<QRCode.Image alt="DiceUI QR Code" />
	...
	<ComponentPreview
		title="Customization"
		description="Mirrors qr-code-customization-demo.tsx. Colours are encoder inputs, not Tailwind classes, so upstream's hex values are kept verbatim."
	>
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div class="flex flex-col items-center gap-2">
				<QRCode.Root value={VALUE} size={150} foregroundColor="#3b82f6" backgroundColor="#f1f5f9">
					<QRCode.Canvas />
					<QRCode.Skeleton />
				</QRCode.Root>
	...
	<ComponentPreview
		title="Overlay"
		description="Mirrors qr-code-overlay-demo.tsx. Every code uses level=&quot;H&quot; so it still scans with up to 30% of it covered."
	>
		<div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
			<div class="flex flex-col items-center gap-2">
	...
	<ComponentPreview
		title="Playground"
		description="Svelte-specific. The code regenerates as soon as any input changes, and clearing the value renders nothing at all rather than erroring."
	...
	<section class="flex flex-col gap-6">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Root</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Canvas</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Svg</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Image</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Overlay</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Skeleton</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">QRCode.Download</h3>
			<p class="text-sm text-muted-foreground">
		...
		<div class="flex flex-col gap-3">
			<h3 class="text-lg font-medium">Error Correction Levels</h3>
			<p class="text-sm text-muted-foreground">
