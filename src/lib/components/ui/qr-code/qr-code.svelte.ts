...
/** Every value {@link QRCodeLevel} accepts, in upstream declaration order (`qr-code.tsx:15`). */
...
/** Builds the encoder options — the port of upstream's `canvasOpts` memo (`:160-173`). */
...
/**
 * The fingerprint of one generation — the port of upstream's `generationKey` memo (`:175-187`),
 ...
 * requested format, or returns `null` when that format has no output yet (FR-006).
 */
...
/**
 * Loads the `qrcode` package lazily, exactly as upstream does (`qr-code.tsx:206`) — the dynamic
 ...
 *
 * Replaces upstream's `Store` + `useSyncExternalStore` + both React contexts (`qr-code.tsx:32-82`),
...
export class QRCodeState {
	...
	/** The key the next generation should produce; `''` disables generation (FR-009). */
	...
	/** The accessible name shared by the canvas and SVG renderers (FR-011). */
	...
	/**
	 * The port of upstream's `onQRCodeGenerate` (`:189-249`). Returns immediately when there is
	 * nothing to encode or when `targetKey` has already been produced (FR-007). A target requested
	...
	async generate(targetKey: string): Promise<void> {
		...
		try {
			...
			// Divergence from upstream (research R-11 / FR-008): a failed regeneration must not leave
	...
	/**
	 * Downloads the current output for `format`. A format with no output yet is a no-op (FR-006).
	 * The port of upstream's download `onClick` body (`:407-426`).
	 */
