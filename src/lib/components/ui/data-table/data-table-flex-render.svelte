...
<script lang="ts" generics="TContext extends object">
	...
	// type and `{@render}`s a snippet (plan.md Divergence 13, research D-03). It renders nothing
	// of its own: no wrapper element, no `data-slot`.
	 ...
	 * function are indistinguishable at runtime (research D-03) — `DataTableColumnDef` is the type
	 * that carries the distinction, and this declared guard re-applies it. Nothing calls the
