<script lang="ts" module>
	...
	export type SortableItemHandleChildProps = {
		type: 'button';
	...
	export type SortableItemHandleProps = WithElementRef<HTMLButtonAttributes, HTMLButtonElement> & {
		/**
		 ...
		 */
		child?: Snippet<[{ props: SortableItemHandleChildProps }]>;
	};
</script>
...
<script lang="ts">
	...
	import { getSortableItemContext } from './sortable.svelte.js';
	...
	// Read before anything else, so a handle with no `<Sortable.Item>` above it fails with the
	...
	const attachHandle = createAttachmentKey();

	function attach(element: Element): (() => void) | void {
		if (!(element instanceof HTMLElement)) return;
		item.setActivator(element);
		return () => item.setActivator(null);
	}
	...
	const handleAttrs = $derived({
		...
		...restProps,
		// The element is already a button, so dnd-kit's `role` is not reproduced here (research R-11).
		...
		[attachHandle]: attach
	} as SortableItemHandleChildProps);
</script>
