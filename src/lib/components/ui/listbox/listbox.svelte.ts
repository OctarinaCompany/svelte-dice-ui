...
export type ListboxItemData = {
	...
	/** The id of the nearest `<Listbox.Group>`, when there is one. */
...
/**
 * Upstream's `useCollection` (`listbox.tsx:177-231`) — a DOM-ordered item registry with group
 * membership. Deliberately mirrors `ComboboxCollection`'s `register()` / `getItems()` contract so
...
/** Upstream `findEnabledItem` (`listbox.tsx:233-271`), ported unchanged. */
...
/** Upstream `getMinItemValue` (`listbox.tsx:273-281`) — the first enabled value. */
...
/** Upstream `getMaxItemValue` (`listbox.tsx:283-292`) — the last enabled value. */
...
/**
 * Upstream `calculateGridLayout` (`listbox.tsx:294-338`), ported verbatim including the 10px
 * same-row tolerance. Measured lazily, from the arrow-key handler only, so no `$derived` ever
 * triggers a synchronous reflow (research R-07).
 */
...
/**
 * The buffered character matcher behind APG typeahead (research R-08). Upstream has no equivalent;
 ...
 * (`listbox.tsx:54-231`) and its whole `onKeyDown` (`listbox.tsx:551-765`). Svelte's signals already
 * give the per-item granularity the external store existed to buy back, so the selection is read
 * straight from the Root's value prop and never mirrored (research R-01).
 */
export class ListboxRootState {
	 ...
	 * makes tabbing back into the listbox land where the user left (FR-020).
	 */
	...
	/**
	 * Upstream `onItemSelect` (`listbox.tsx:462-515`). `onSelect` is invoked directly rather than
	...
	/** `Ctrl`/`Cmd`+`A` — every enabled value at once. A no-op outside `multiple` (FR-007). */
	selectAll(): void {
		if (this.disabled || !this.multiple) return;
		this.#props.setValues(this.collection.getEnabledItems().map((item) => item.value));
	}

	/** The contiguous enabled slice between two values, inclusive. `multiple` only (FR-008). */
	...
	/** `Escape`, and focus leaving the root: the selection is deliberately untouched (FR-019). */
	clearFocus(): void {
		this.focusedValue = null;
		this.highlightedValue = null;
	}

	/** Upstream `onFocus` (`listbox.tsx:767-796`) — first entry lands on the remembered option. */
	...
	/** Upstream `onBlur` (`listbox.tsx:798-808`) — only when focus leaves the root subtree. */
	...
	onRootKeydown(event: KeyboardEvent): void {
		...
		// stays matchable past its first word (FR-021).
		...
		if (isSelectionKey) {
			...
			// `multiple`: with `multiple` on, the modifier never changes the outcome (research R-09).
