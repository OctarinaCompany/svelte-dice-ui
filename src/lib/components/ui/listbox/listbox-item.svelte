...
<script lang="ts">
	...
	// The group is the one context an item may legitimately be without.
	const group = hasListboxGroupContext() ? getListboxGroupContext('<Listbox.Item>') : undefined;
	...
	// tracked read of the list this effect appends to would re-run it forever (research R-11).
	$effect(() =>
