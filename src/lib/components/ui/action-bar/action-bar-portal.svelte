<script lang="ts" module>
	...
	export type ActionBarPortalProps = {
		/**
...
<script lang="ts">
	...
	let { to, children }: ActionBarPortalProps = $props();
	 ...
	 * `Element | DocumentFragment | null` (action-bar.tsx:120). A `display: contents` host appended
	 * to the fragment bridges the gap in five lines while `bits-ui` still performs the mount, keeps
	 * `getAllContexts()` propagation and stays a no-op outside the browser (research R-02).
	 */
	...
	$effect(() => {
		...
		host.setAttribute('data-slot', 'action-bar-portal-host');
