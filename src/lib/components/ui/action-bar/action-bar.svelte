...
<script lang="ts">
	...
	// Upstream's document-level `Escape` listener (action-bar.tsx:162-178), attached only while the
	...
	const rootAttrs = $derived({
		...
		// The caller's declarations come last, so they win over the viewport-edge ones (FR-003).
