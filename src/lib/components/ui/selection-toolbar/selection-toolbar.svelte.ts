...
/** Every value `side` accepts. Upstream `SIDE_OPTIONS` (selection-toolbar.tsx:28). */
 ...
 * write path and it always notifies `onOpenChange` (FR-002). Must be constructed during component
...
export class SelectionToolbarRootState {
	...
	/** rAF token of the deferred `mouseup` read (460-464), cancelled by the effect's teardown. */
	...
	constructor(props: SelectionToolbarRootStateProps) {
		 ...
		 * browser's ranges, and FR-009 would be satisfied only by luck. This listener is synchronous
		 * on the press, exactly like upstream's, and `clearSelection()` is idempotent, so the two
		 * paths compose. `pointerdown` rather than upstream's `mousedown`: it is the same moment for
		 * a mouse and it also covers pen and touch, which `mousedown` reports late or not at all.
		 */
		$effect(() => {
			if (!this.open) return;
...
/**
 * Read the root's state, throwing when there is no `<SelectionToolbar>` ancestor (FR-016).
