...
<script lang="ts">
	 ...
	 * `Element | DocumentFragment | null` (selection-toolbar.tsx:107). A `display: contents` host
	 * appended to the fragment bridges the gap while `bits-ui` still performs the mount, keeps
	 * `getAllContexts()` propagation and stays a no-op outside the browser (research R-08).
	 */
