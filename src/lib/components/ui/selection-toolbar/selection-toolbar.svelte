<script lang="ts" module>
	...
	/** A single collision boundary. Upstream `Boundary` (selection-toolbar.tsx:33). */
...
<script lang="ts">
	 ...
	 * computes from the same `size` middleware (divergence D-5, research R-05). The caller's own
	...
	/** `Escape` (FR-008) drops the browser's ranges before the layer closes the surface. */
	function handleEscapeKeydown() {
		rootState.clearSelection();
	}

	/**
	 * A pointer press outside the surface (FR-009). The dismissible layer raises this from
