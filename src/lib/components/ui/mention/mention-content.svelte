...
<script lang="ts">
	...
	/** Upstream's `rtlAwareAlign` (`mention-content.tsx:78-81`). */
	 ...
	 * plus `fitViewport`'s clamp (divergence D-5) — `bits-ui` does not expose floating-ui's `size`
	 ...
	 * of sight, which is upstream's `visuallyHidden` branch (`mention-content.tsx:112`).
	 */
...
<PopoverPrimitive.Content
	...
	{...restProps}
	class={cn(
		'relative z-50 max-h-fit min-w-(--dice-anchor-width) origin-(--dice-transform-origin) overflow-hidden rounded-lg bg-popover p-1 text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95',
		className
	)}
>
	{@render children?.()}
