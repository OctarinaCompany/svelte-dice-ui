...
<script lang="ts">
	...
	/**
	 * Divergence from upstream (`mention-item.tsx:120-128`), per FR-028: a `readonly` root makes
	 * highlight movement inert, and the keyboard already honours that (`onInputKeydown` returns early
	 * for the navigation keys), so the pointer must not be the one way back in.
	 */
