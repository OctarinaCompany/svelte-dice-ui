...
export class MentionCollection {
	...
	/** Upstream `getEnabledItems` (`mention-root.tsx:251-253`). */
...
export class MentionRootState {
	...
	/**
	 * The scored view of the collection, reusing the already-ported filter module (research R-03).
	...
	/** Upstream `setValue` (`mention-root.tsx:216-220`). */
	...
	/** Upstream `setInputValue` (`mention-root.tsx:221-225`). */
	setInputValue(next: string): void {
		this.#props.setInputValue(next);
	}

	/**
	 * Upstream `onOpenChange` (`mention-root.tsx:255-273`): a request to open while a non-empty
	...
	/** Upstream's local `onMenuClose` (`mention-input.tsx:598-602`). */
	closeMenu(): void {
		this.setOpen(false);
		this.highlightedElement = null;
		this.search = '';
	...
	/**
	 * Upstream `onMentionAdd` (`mention-root.tsx:287-335`) — the splice that inserts
	...
	/** Upstream `onMentionsRemove` (`mention-root.tsx:337-367`). */
	removeMentions(spans: readonly MentionSpan[]): void {
		this.mentions = removeMentionSpans(this.mentions, spans);
	}

	/**
	 * Upstream `onMentionUpdate` (`mention-input.tsx:136-264`): decide whether the caret sits in an
	...
	updateTrigger(element: MentionField, caret: number | null = null): boolean {
		...
		// store is a `$derived`, so the count is already fresh and the check is synchronous (R-03).
	...
	/** Upstream `onChange` (`mention-input.tsx:266-306`). */
	...
	/** Upstream `onBeforeInput` (`mention-input.tsx:690-735`) — mobile `deleteContentBackward`. */
	...
	/** Upstream `onCut` (`mention-input.tsx:315-354`). */
	...
	/** Upstream `onSelect` / `onFocus` / `onClick` (`mention-input.tsx:308-313, 356-361, 737-742`). */
	onInputCaretMove(input: MentionField): void {
		this.updateTrigger(input);
	}

	/**
	 * Upstream `onPointerDown` (`mention-input.tsx:744-779`): a press inside an inserted mention is
	...
	async onInputPointerDown(event: PointerEvent, input: MentionField): Promise<void> {
		...
		event.preventDefault();
		// The caret write has to land after the browser's own pointer handling (research R-08).
		await tick();
		if (this.#destroyed) return;
	...
	/**
	 * Upstream `onKeyDown` (`mention-input.tsx:363-687`): atomic-mention caret navigation and
	...
	onInputKeydown(event: KeyboardEvent, input: MentionField): void {
		...
		// (contracts/public-api.md, keyboard table).
		const consumesKey =
			isNavigationKey &&
			event.key !== 'Tab' &&
	...
	/**
	 * Upstream `onPaste` (`mention-input.tsx:781-978`): split the pasted text on the trigger and
	 * rebuild the longest word combination that matches a registered item, keeping everything else
	 * verbatim. Best-effort by upstream's own admission (research R-12).
	 *
	 * **Divergence from upstream** (FR-026): upstream returns early when the pasted text carries no
	...
	/** Upstream's local `onItemSelect` (`mention-input.tsx:604-618`), also used by an item click. */
	...
	/** Upstream `mention-input.tsx:371-446`. */
	...
	/** Upstream `mention-input.tsx:448-482`. */
	#deleteSelection(
		event: KeyboardEvent,
		input: MentionField,
		caret: number,
	...
	/** Upstream `mention-input.tsx:484-580`. */
	...
	/**
	 * The forward-delete counterpart of {@link #backspaceMention} (FR-023c).
	 *
	 ...
	 * mention's trailing space, and removing only that space is the mirror image of FR-023a.
	 */
	...
	/**
	 * Upstream's `requestAnimationFrame` hop in `onOpenChange` (`mention-root.tsx:261-266`), waiting
