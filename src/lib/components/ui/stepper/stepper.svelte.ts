import {
	...
	wrapArray
} from '$lib/components/ui/action-bar/action-bar-roving-focus.svelte.js';
 ...
 *
 * Upstream builds a hand-rolled `useSyncExternalStore` store (`stepper.tsx:141-330`) with
 ...
 * `speed-dial`'s {@link DomOrderedCollection}, and the pure roving-focus helpers `focusFirst`,
 * `wrapArray` and `getDirectionAwareKey` come from `action-bar`'s roving-focus module (research
 * R-03, R-04).
 */
...
/** Upstream `EVENT_OPTIONS` (stepper.tsx:28). */
 ...
 *
 * The `rtl` swap is delegated to `action-bar`'s shared `getDirectionAwareKey`; the key map itself is
 * stepper's own, because it adds `PageUp`/`PageDown` that the toolbar pattern does not have
 * (research R-04).
 */
...
export class StepperRootState {
	 ...
	 * `Stepper.Next` all index into (research R-03).
	 */
	 ...
	 * writes the stale result unconditionally (283-298), which spec Edge Case 4 forbids (R-05).
	 */
	...
	/**
	 * Upstream `setStateWithValidation` (283-298), plus the staleness guard from research R-05.
	 * A validator that rejects — or throws synchronously — blocks the move exactly like one
	 * resolving `false` (295-297).
	 */
 ...
 * map has no `PageUp`/`PageDown` (research R-04, plan.md "Bespoke behaviour justification"). What
 * *is* reusable was reused: the registry is `DomOrderedCollection` and the three pure helpers are
 * imported from that same module.
 */
