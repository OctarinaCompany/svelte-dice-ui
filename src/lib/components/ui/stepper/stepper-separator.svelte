<script lang="ts" module>
	...
	/** Upstream's inline separator `cn()` (stepper.tsx:1075-1079). */
	...
	> & {
		 ...
		 *
		 * Present in the upstream source (stepper.tsx:1028-1030, 1052) but missing from its documented
		 * types file; the source wins (research R-12).
		 * @default false
		 */
		forceMount?: boolean;
		/**
...
<script lang="ts">
	...
	// The `'separator'` variant is what keeps the line *after* the active step unfilled (research
	// R-06): a separator belongs to the step before it, not to the step it points at.
