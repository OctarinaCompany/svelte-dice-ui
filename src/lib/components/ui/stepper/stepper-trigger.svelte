...
<script lang="ts">
	import { focusFirst } from '$lib/components/ui/action-bar/action-bar-roving-focus.svelte.js';
	 ...
	 * (899-907), applied to every activation path (data-model.md §5).
	 */
	...
	const triggerAttrs = $derived({
		...
		// moment a Title/Description mounts, and a dangling IDREF is ignored by browsers (research
		// R-11).
		...
		class: cn(
			...
			// Description parts, so the rendered result is identical (research R-10).
