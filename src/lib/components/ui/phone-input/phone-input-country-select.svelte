<script lang="ts" module>
	...
	export type PhoneInputCountrySelectProps = Omit<PopoverPrimitive.RootProps, 'children'> & {
		 ...
		 * value wins over it, matching upstream's spread order (research R-12).
		 *
...
<script lang="ts">
	...
	function handleCloseAutoFocus(event: Event) {
		...
		// `requestAnimationFrame` focus only runs on the select path (research R-08).
...
<Popover.Root open={isOpen} onOpenChange={handleOpenChange} {onOpenChangeComplete} {...restProps}>
	...
	>
		{#if !root.selectedCountry}
			<div data-slot="phone-input-country-swatch" class="h-4 w-6 rounded-sm bg-muted/50"></div>
		{:else if root.showFlag && root.selectedCountry.flag}
			<div class="w-6 text-lg leading-none">{root.selectedCountry.flag}</div>
		{/if}
