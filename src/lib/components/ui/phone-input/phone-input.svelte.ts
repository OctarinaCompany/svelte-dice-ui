 ...
 * `PhoneInputContext` (phone-input.tsx:377-443) — both exist only because React has no fine-grained
 * reactivity, and both are provided by the same root and read by the same two parts (research
 * R-03). Reactive inputs arrive as getter functions rather than snapshots (`CLAUDE.md` §4), and the
 * value is read straight from the root's `$bindable` props, which is what lets an authoritative
 * parent decline a write.
 */
export class PhoneInputRootState {
	 ...
	 * which always leads with one once it holds a digit. It gates auto-detection (research R-06).
	 */
	...
	/**
	 * Upstream's detection effect (phone-input.tsx:562-574), reproduced verbatim including both of
	 ...
	 * (research R-05). The write is guarded by `detected.code !== country`, so the effect that calls
...
/** Read the root's state, throwing when there is no `<PhoneInput.Root>` ancestor (FR-012). */
