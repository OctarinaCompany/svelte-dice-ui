<script lang="ts" module>
	...
	> & {
		 ...
		 * own demo renders the flag-less swatch; runtime behaviour wins here (research R-04).
		 *
		 * @default ""
		 */
		 ...
		 * `<PhoneInput.Field>` (research R-12).
		 *
...
<script lang="ts">
	...
	const phoneState = setPhoneInputContext(
		new PhoneInputRootState({
			...
			// change (phone-input.tsx:522-536).
			setValue: (next) => {
	...
	// Upstream's detection effect, dependencies and all (research R-05). It writes the country it
