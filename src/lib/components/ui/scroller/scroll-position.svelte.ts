 ...
 *
 * This module is deliberately standalone: it imports nothing from the rest of the `scroller` folder,
 * knows nothing about masks, direction context or markup, and is exported from the barrel so the
 * wave-3 `scroll-spy` and `tour` ports can consume it directly instead of re-deriving it (FR-010).
 * `readScrollMetrics`, `computeAxisOverflow` and `observeScrollPosition` are the stability promise to
 * those ports — changing their signatures is a breaking change.
 */
...
export type ComputeAxisOverflowOptions = {
	 ...
	 * `-(scrollWidth - clientWidth)`) models, so no browser-model sniffing is needed (research R-04).
	 * @default 'ltr'
	 */
	dir?: 'ltr' | 'rtl';
};
 ...
 * would leave a stale edge cue (divergence D-02, research R-07).
 *
