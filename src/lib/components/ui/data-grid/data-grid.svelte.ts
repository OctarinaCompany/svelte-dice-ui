 ...
 * `batch()` and the prop plumbing all disappear (plan.md Divergences 1 and 2).
 *
 * The table instance is created once and never re-created; reactivity flows through getters
 * installed on `table.options` after `createTable`, the bridge `data-table` already proved.
 */
export class DataGridState<TData extends RowData> {
	...
	/** Slices this class does not own (`columnSizing`, `columnSizingInfo`, `expanded`, …). */
	...
	rowHeight = $state<RowHeightValue>('short');
	...
	constructor(options: CreateDataGridOptions<TData>) {
		...
		Object.defineProperties(table.options, {
			...
			columnResizeDirection: { get: () => this.dir, enumerable: true, configurable: true }
		});
	...
	#setColumnPinning(updater: Updater<ColumnPinningState>): void {
		this.columnPinning = functionalUpdate(updater, this.columnPinning);
	}

	#setColumnOrder(updater: Updater<ColumnOrderState>): void {
		this.columnOrder = functionalUpdate(updater, this.columnOrder);
	}

	#setColumnVisibility(updater: Updater<VisibilityState>): void {
		this.columnVisibility = functionalUpdate(updater, this.columnVisibility);
	}
	...
	/**
	 * The whole grid key contract, in the precedence order of `contracts/keyboard.md`:
	 ...
	 * because `createDataGrid` runs before any DOM exists (FR-037).
	 */
	applyAutoFocus(): void {
