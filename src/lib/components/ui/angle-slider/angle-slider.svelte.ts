...
/** `min` fallback — upstream `angle-slider.tsx:188`. */
export const DEFAULT_MIN = 0;
/** `max` fallback — upstream `angle-slider.tsx:189`. */
export const DEFAULT_MAX = 100;
/** `step` fallback — upstream `angle-slider.tsx:190`. */
export const DEFAULT_STEP = 1;
/** Dial **radius** in px — upstream `angle-slider.tsx:192`. */
export const DEFAULT_SIZE = 60;
/** Track stroke width in px — upstream `angle-slider.tsx:193`. */
export const DEFAULT_THICKNESS = 8;
/** `-90` puts `min` at 12 o'clock — upstream `angle-slider.tsx:194`. */
export const DEFAULT_START_ANGLE = -90;
/** `270` closes the full circle — upstream `angle-slider.tsx:195`. */
...
/** Keys that move by `10 · step` — upstream `angle-slider.tsx:18`. */
export const PAGE_KEYS = ['PageUp', 'PageDown'] as const;
/** Keys that move by `step` — upstream `angle-slider.tsx:19`. */
...
/** Upstream `clamp` — `angle-slider.tsx:30-32`, with the tuple flattened into two arguments. */
export function clamp(value: number, min: number, max: number): number {
	return Math.min(max, Math.max(min, value));
}

/** Upstream `getDecimalCount` — `angle-slider.tsx:64-66`. */
export function getDecimalCount(value: number): number {
	return (String(value).split('.')[1] ?? '').length;
}

/** Upstream `roundValue` — `angle-slider.tsx:68-71`. */
...
/**
 * Upstream's snap-then-clamp pair, inlined in `updateValue` at `angle-slider.tsx:266-273`.
...
/** Upstream `getNextSortedValues` — `angle-slider.tsx:34-42`. */
...
/** Upstream `getStepsBetweenValues` — `angle-slider.tsx:44-49`. */
...
/** Upstream `hasMinStepsBetweenValues` — `angle-slider.tsx:51-62`. */
...
/** Upstream `getClosestValueIndex` — `angle-slider.tsx:73-78`. */
 ...
 * `(endAngle - startAngle + 360) % 360 || 360`, repeated at `angle-slider.tsx:309`, `321`, `597`
...
/**
 * Upstream `getValueFromPointer` — `angle-slider.tsx:296-315`.
 *
...
/** Upstream `getAngleFromValue` — `angle-slider.tsx:316-325`. */
...
/** Upstream `getPositionFromAngle` — `angle-slider.tsx:326-334`. Offsets from the dial centre. */
 ...
 * (`angle-slider.tsx:87-159`, `236-341`). One instance per `<AngleSlider.Root>`, published on
 * context and read by every part.
 */
export class AngleSliderRootState {
	...
	/** Upstream `addThumb` — `angle-slider.tsx:258-261`. */
	register(index: number, element: HTMLElement): void {
		this.thumbs.set(index, { index, element });
	}

	/** Upstream `removeThumb` — `angle-slider.tsx:262-265`. */
	 ...
	 * layout read ever happens inside an `$effect` (FR-020).
	 */
	...
	/**
	 * Upstream `updateValue` — `angle-slider.tsx:266-295`. Snaps, rounds, clamps and re-sorts, then
	...
	/** The thumb whose wrapper contains `target`, if any — upstream `angle-slider.tsx:500-504`. */
	...
	/** Upstream `onSliderStart` — `angle-slider.tsx:413-423`. */
	...
	/** Upstream `onSliderMove` — `angle-slider.tsx:425-433`. */
	...
	/** Upstream `onSliderEnd` — `angle-slider.tsx:435-447`. Commits only on a real change. */
	endSlide(): void {
		if (!this.interactive) return;
	...
	/**
	 * Upstream `onKeyDown` — `angle-slider.tsx:449-486`, with two deliberate divergences:
	 *
 ...
 * (`angle-slider.tsx:126-143`). Throws when the part is rendered outside `<AngleSlider>`.
 */
