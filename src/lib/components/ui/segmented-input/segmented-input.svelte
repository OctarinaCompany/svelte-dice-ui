<script lang="ts" module>
	...
	> & {
		 ...
		 * fallback resolves from the document rather than from this element (divergence D-03).
		 */
...
<script lang="ts">
	 ...
	 * (radix/ui/segmented-input.tsx:108) and FR-001 requires it. Spreading it keeps the emitted value
	 * identical while staying out of the compiler's static `a11y_role_supports_aria_props` analysis,
	 * which only inspects attributes written literally on the element — the same treatment
	 * `checkbox-group.svelte` gives its own superset ARIA.
	 */
