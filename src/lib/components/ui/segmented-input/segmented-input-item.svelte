<script lang="ts" module>
	 ...
	 * union without a cast (Principle VI).
	 */
	...
	> & {
		...
		/** Any native input type except `file` (D-04). */
...
<script lang="ts">
	...
	function registerSegment(element: Element) {
		...
		root.nav.register(id, element, {
			...
			setValue: (next) => {
				...
				// wins — Svelte restores the input from the getter (research R-11).
				element.value = next;
				value = next;
	...
	// documented opt-out for both enhancements (research R-12).
