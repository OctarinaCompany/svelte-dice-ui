 ...
 * (`radix/ui/segmented-input.tsx:78-100`), which has no Svelte equivalent — an unregistered segment
 * (`index === -1`) reads `"isolated"`, which is also the value every segment carries for the first
 * frame, before its registration effect has run (research R-02, divergence D-02).
 */
...
/**
 * The key → intent table of `contracts/public-api.md` §5. Vertical navigation is
 * direction-independent, matching every ARIA vertical-orientation pattern; horizontal navigation
 * inverts under `dir="rtl"`. Anything else returns `null` and the browser keeps the key.
 */
...
/**
 * Split a pasted string into one part per target segment (research R-10).
 *
...
/** `null` means the input type has no caret at all, which counts as "at both edges" (R-08). */
 ...
 * behaviour (FR-015, research R-13).
 *
 * ```ts
...
export class SegmentNavigation {
	...
	/** Composed rather than re-written — the collection Speed Dial exported for reuse (R-03). */
	 ...
	 * with nothing selected, so editing the middle of a segment still works (research R-08, D-07);
	 * `Home`/`End` jump unconditionally (R-09). Modified keystrokes are left alone — `Ctrl+Home`
	 * stays the document shortcut and `Shift+Arrow` stays text selection.
	 */
	 ...
	 * (research R-10). Segments before the focused one are never touched, disabled and read-only
