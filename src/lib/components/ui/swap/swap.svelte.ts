...
/** A runes reader over `(prefers-reduced-motion: reduce)`, SSR-safe. */
export class ReducedMotionReader {
	current: boolean = $state(readReducedMotion());
...
/** Reusable by later ports. Must be called during component initialisation. */
export function useReducedMotion(): ReducedMotionReader {
	return new ReducedMotionReader();
}
