...
<script lang="ts">
	...
	const inputAttrs = $derived({
		...
		// themselves, and a second writer would snap the caret back to the end (research R-03/R-04).
		[attach]: syncValue,
		...
		oncompositionend: (event: MaskInputEvent<CompositionEvent>) => maskState.oncompositionend(event)
