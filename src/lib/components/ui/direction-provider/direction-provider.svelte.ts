...
/** `'ltr' | 'rtl'` — mirrors `@diceui/shared`'s `Direction`. */
...
export function getDirectionContext(): DirectionProviderState {
	if (!hasDirectionContext()) {
		throw new Error('`<Part>` must be used within `<DirectionProvider>`.');
	}
