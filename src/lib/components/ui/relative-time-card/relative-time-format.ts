 ...
 * (`relative-time-card.tsx:13-44`) are reproduced exactly; only the machinery differs.
 *
 * Nothing here throws. Upstream crashes on an unparseable `date` and on an unknown IANA time zone;
 * both are rendered states here instead (research R-07).
 */
 ...
 * the attribute is simply omitted rather than `toISOString()` throwing (research R-07).
 */
 ...
 * `Date.prototype.toLocaleDateString` respectively (research R-07).
 */
