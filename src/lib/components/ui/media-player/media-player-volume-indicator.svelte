...
<script lang="ts">
	...
	// Announce this indicator so the root stops rendering its own (research R-12). The one the root
...
{#if root.volumeIndicatorVisible}
	...
	{:else}
		<div bind:this={ref} {...indicatorAttrs}>
			...
			{:else}
				<div
					class="flex animate-in flex-col items-center gap-3 rounded-lg bg-background/30 px-6 py-4 text-foreground backdrop-blur-xs duration-200 fade-in-0 zoom-in-95"
				>
