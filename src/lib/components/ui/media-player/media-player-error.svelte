<script lang="ts" module>
	...
	/** Verbatim from upstream's `labelMap` (media-player.tsx:1030-1035). */
	...
	/** Verbatim from upstream's `descriptionMap` (media-player.tsx:1045-1053). */
...
<script lang="ts">
	...
	const errorLabel = $derived(
		label ?? (activeError ? (ERROR_LABELS[activeError.code] ?? 'Playback Error') : 'Playback Error')
	);
