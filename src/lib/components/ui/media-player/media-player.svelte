<script lang="ts" module>
	 ...
	 * both to carry a value, exactly as upstream does (media-player.tsx:157-175).
	 */
	...
	> & {
		 ...
		 * directions (FR-024); only the layout mirrors.
		 *
		...
		/** Fired whenever the document's fullscreen element changes. */
...
<script lang="ts">
	...
	const rootAttrs = $derived({
		...
		class: cn(
			'dark relative isolate flex size-full flex-col overflow-hidden rounded-lg bg-background outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 data-disabled:pointer-events-none data-disabled:opacity-50 [&_video]:relative [&_video]:object-contain',
...
<!--
	...
	*and* on the media element. The description spells out the shortcut set, which differs between
	video and audio because `←`/`→` need `Shift` on audio (FR-004).
-->
	...
	{:else}
		<div bind:this={ref} {...rootAttrs}>
			...
			<!--
				...
				root only fills the gap when nothing else does (research R-12).
			-->
