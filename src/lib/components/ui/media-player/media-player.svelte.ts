...
/**
 * A caller-supplied playback-quality option (research R-07). Upstream reads these from
...
/** Upstream `SPEEDS` (media-player.tsx:70). */
...
/** Which parts announce themselves to the root so it can stop duplicating them (research R-12). */
...
export class MediaPlayerState {
	...
	/** Upstream's `bufferedProgress` memo (media-player.tsx:1695-1709), as a `0…1` fraction. */
	...
	attachMedia(): () => void {
		...
		// `loop` has no Svelte binding and can be flipped by the caller at any time (FR-017).
		const observer =
	...
	attachRoot(): () => void {
		...
		const onFullscreenChange = () => {
			this.fullscreen = document.fullscreenElement === this.rootEl;
			this.#props.getCallbacks().onFullscreenChange?.(Boolean(document.fullscreenElement));
		};
	...
	#syncTextTracks() {
		...
		for (let index = 0; index < (list?.length ?? 0); index += 1) {
			...
			// thumbnail metadata have to be `hidden` — parsed but not painted (research R-05).
	 ...
	 * exists (research R-12). The increment is untracked: it is written from a caller's `$effect`,
	 ...
	 * `speeds[0]` in either direction — upstream's `indexOf` behaviour (media-player.tsx:475-499).
	 */
	...
	async toggleFullscreen() {
		...
		try {
			...
			// A refused or unsupported transition leaves `data-state="windowed"` (research R-06).
		}
	}
	...
	onRootKeydown(event: KeyboardEvent) {
		...
		switch (event.key.toLowerCase()) {
			...
			case 'd':
				// The shortcut is owned by a mounted `<MediaPlayer.Download>` (research R-12).
	...
	onRootKeyup(event: KeyboardEvent) {
		...
		// keydown branch that performs the change (contract §12).
 ...
 * @throws when the part is rendered outside `<MediaPlayer>` (contract §13).
 */
