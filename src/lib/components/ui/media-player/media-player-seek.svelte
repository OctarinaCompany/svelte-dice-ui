...
<script lang="ts">
	...
	// plain non-reactive locals (research R-16).
	 ...
	 * boundary. Only the maths is bespoke — the portal is composed (research R-04).
	 */
	...
	function onValueChange(value: number) {
		...
		// drag does not queue dozens of them (SC-003).
		if (seekThrottleId !== null) cancelAnimationFrame(seekThrottleId);
			...
			{#snippet children({ thumbItems })}
				...
				{#each thumbItems as thumb (thumb.index)}
					...
					<Slider.Thumb
						...
						aria-controls={root.mediaId}
						aria-valuetext="{currentLabel} of {durationLabel}"
						class="relative z-10 block size-2.5 shrink-0 rounded-full bg-primary shadow-sm ring-ring/50 transition-[color,box-shadow] will-change-transform hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
					/>
				{/each}
			{/snippet}
		</Slider.Root>
