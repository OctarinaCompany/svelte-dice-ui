 ...
 * component can compose it instead of re-deriving the absolute-direction filter (research R-19).
 */
...
/**
 * Upstream's four `coordinateGetter` inequalities (`kanban.tsx:88-109`), kept verbatim.
 *
 * The direction is absolute — screen left is screen left whatever the board's `orientation` is
 * (FR-006). Only the caller mirrors it for `dir="rtl"`.
 */
 ...
 *
 * Candidate rules (research R-06): dragging a **column** targets only other columns; dragging an
