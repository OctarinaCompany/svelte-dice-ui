...
<script lang="ts">
	...
	// pointer (research R-07). The write is untracked because it reads the same field it writes.
	$effect(() => {
	 ...
	 * the gap while `bits-ui` still performs the mount (research R-09).
	 */
	...
	const variant = $derived<KanbanOverlayVariant>(
		activeId !== null && root.isColumn(activeId) ? 'column' : 'item'
	);
