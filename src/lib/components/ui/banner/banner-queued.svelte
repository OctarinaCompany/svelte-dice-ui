...
<script lang="ts">
	...
	// Upstream `useEffect` (banner.tsx:373-376): flip to the entered position one frame after mount.
	$effect(() => {
	...
	// Upstream `useLayoutEffect` (banner.tsx:378-382): report this banner's measured height, so the
	// container and its siblings can offset around it. The write is `untrack`ed because `setHeight`
	// reads `heights` to short-circuit an unchanged value before writing it back (`CLAUDE.md` §4).
	$effect.pre(() => {
	...
	// actually was (banner.tsx:369-371). Read, not written, by the transform below.
	$effect.pre(() => {
		if (!removing) frozenOffset = offset;
	});

	// Upstream `useEffect` (banner.tsx:384-392): drop the height so the container shrinks and the
