<script lang="ts" module>
	...
	export type BannerRootProps = WithElementRef<HTMLAttributes<HTMLDivElement>> & {
		/** Controlled open state. See `contracts/public-api.md` §8 for the controlled/uncontrolled idiom. */
		open?: boolean;
		/** @default true */
		defaultOpen?: boolean;
		/** Called whenever the banner opens or closes, in both controlled and uncontrolled modes. */
		onOpenChange?: (open: boolean) => void;
		/** Fires on dismissal, but **only** while registered inside a `<Banner.Queue>` (research R-18). */
		onDismiss?: () => void;
		/** @default 'default' */
		variant?: BannerVariant;
		/** Determines queue ordering. Read only while registered inside a `<Banner.Queue>` (R-18). */
		priority?: number;
		/** Auto-dismiss delay in ms. Read only while registered inside a `<Banner.Queue>` (R-18). */
		duration?: number;
		/** @default true */
		dismissible?: boolean;
		/**
...
<script lang="ts">
	...
	// this effect must not subscribe to (research R-04).
	$effect(() => {
